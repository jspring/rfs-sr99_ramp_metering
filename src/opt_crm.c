...
//This part aggregate onramp data for each section
	...
	for(i=0;i<NumOnRamp;i++){
		onramp_out[cycle_index][i].agg_vol = Mind(MAX_OR_RAMP_FLOW_PER_LANE*6, Maxd(controller_onramp_data[onrampCTidx[i]].agg_vol,5));
...
//This part aggregate onramp data for each section <--- match number of off-ramp by number of on-ramp 		 
	...
	for(i=0;i<NumOnRamp;i++){ 
		if (offrampCTidx[i] != -1.0){//<-- impute data here
			offramp_out[cycle_index][i].agg_vol = Mind(MAX_FR_RAMP_FLOW_PER_LANE*6, Maxd(controller_offramp_data[offrampCTidx[i]].agg_vol,5));
